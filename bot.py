...
async def slash_daily(interaction: discord.Interaction):
    ...
    if eco["last_daily"] and (now - datetime.fromisoformat(eco["last_daily"])).total_seconds() < 86400:
        ...
        m = rem // 60
        embed = discord.Embed(title="⏰ Daily Already Claimed", description=f"Come back in **{h}h {m}m**!", color=0xe74c3c)
        return await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def slash_work(interaction: discord.Interaction):
    ...
    if eco["last_work"] and (now - datetime.fromisoformat(eco["last_work"])).total_seconds() < 3600:
        ...
        embed = discord.Embed(title="⏰ Still Recovering", description=f"Rest for **{m}m {s}s** before working again.", color=0xe74c3c)
        return await interaction.response.send_message(embed=embed, ephemeral=True)
    jobs = [
...
async def slash_deposit(interaction: discord.Interaction, amount: str):
    ...
    await interaction.response.send_message(embed=success_embed("Deposited!", f"**{dep:,} coins** deposited to your bank.\n\n💵 Wallet: {eco['balance']:,}\n🏦 Bank: {eco['bank']:,}"))
...
async def slash_withdraw(interaction: discord.Interaction, amount: str):
    ...
    eco["bank"] = bank - wd
    eco["balance"] += wd
    await interaction.response.send_message(embed=success_embed("Withdrawn!", f"**{wd:,} coins** moved to wallet.\n\n💵 Wallet: {eco['balance']:,}\n🏦 Bank: {eco['bank']:,}"))
...
async def slash_gamble(interaction: discord.Interaction, amount: str):
    ...
    if bet <= 0:
        return await interaction.response.send_message(embed=error_embed("Invalid Amount"), ephemeral=True)
    if eco["balance"] < bet:
        return await interaction.response.send_message(embed=error_embed("Insufficient Funds", f"Only **{eco['balance']:,}**."), ephemeral=True)
    win = random.random() > 0.5
    if win:
        eco["balance"] += bet; eco["total_earned"] += bet
...
async def slash_slots(interaction: discord.Interaction, bet: int = 50):
    ...
    if r[0] == r[1] == r[2]:
        ...
        win = bet * multi; eco["balance"] += win - bet; eco["total_earned"] += win - bet
        result = f"🎉 **JACKPOT! {r[0]}** — Won **{win:,}**!"
        color = 0xf1c40f
    elif r[0] == r[1] or r[1] == r[2] or r[0] == r[2]:
        win = bet * 2; eco["balance"] += win - bet; eco["total_earned"] += win - bet
        result = f"✅ **Two of a kind!** — Won **{win:,}**!"
        color = 0x2ecc71
    else:
        eco["balance"] -= bet; result = f"❌ **No luck!** — Lost **{bet:,}**."; color = 0xe74c3c
...
async def slash_rob(interaction: discord.Interaction, member: discord.Member):
    ...
    now = datetime.now()
    if robber.get("last_rob") and (now - datetime.fromisoformat(robber["last_rob"])).total_seconds() < 3600:
        diff = 3600 - (now - datetime.fromisoformat(robber["last_rob"])).total_seconds()
        m, s = divmod(int(diff), 60)
    ...
    if success:
        ...
        embed = discord.Embed(title="🚨 Caught Red-Handed!", description=f"{interaction.user.mention} got caught robbing **{member.display_name}** and paid a **{fine:,} coin** fine! 🚔", color=0xe74c3c)
    await interaction.response.send_message(embed=embed)
...
async def slash_fish(interaction: discord.Interaction):
    ...
    catches = [
        ...
        ("💎 Treasure", 500, 1000), ("🎣 Nothing", 0, 0),
        ("👟 Old Boot", -10, -5), ("🐙 Octopus", 80, 200)
    ]
    catch, min_coins, max_coins = random.choices(catches, weights=[25, 20, 15, 10, 5, 20, 3, 2])[0]
    if max_coins > 0:
        coins = random.randint(min_coins, max_coins)
        eco["balance"] += coins; eco["total_earned"] += coins
        ...
        embed = discord.Embed(
            ...
            description=f"You caught: **{catch}**!\nBetter luck next time!",
            color=0x95a5a6
        )
    await interaction.response.send_message(embed=embed)
...
async def slash_mine(interaction: discord.Interaction):
    ...
    finds = [
        ...
        ("💀 Nothing found", 0, 0),
        ("🧨 Cave-in!", -50, -20),
        ("🔮 Rare Crystal", 300, 600),
        ("⚡ Energy Stone", 150, 300),
    ]
    find, min_c, max_c = random.choices(
        finds, weights=[5, 15, 20, 30, 18, 5, 4, 3]
    )[0]
    if max_c > 0:
        ...
        eco["balance"] = max(0, eco["balance"] - loss)
        embed = discord.Embed(
            title="⛏️ Mining Accident!",
            description=f"**{find}**!\nLost **{loss:,} coins** in damages.\n**Balance:** {eco['balance']:,}",
            color=0xe74c3c
        )
    else:
        embed = discord.Embed(
            title="⛏️ Mining Result!",
            description=f"**{find}**! You found nothing this time.",
            color=0x95a5a6
        )
    await interaction.response.send_message(embed=embed)
...
async def slash_richlist(interaction: discord.Interaction):
    if not economy_data:
        return await interaction.response.send_message(
            embed=error_embed("No Data", "No economy data yet!"), ephemeral=True
        )
...
async def slash_givecoins(interaction: discord.Interaction, member: discord.Member, amount: int):
    ...
    await interaction.response.send_message(
        embed=success_embed("Coins Given", f"Gave **{amount:,} coins** to {member.mention}.\nNew balance: **{eco['balance']:,}**")
    )
 
@tree.command(name="heist", description="Start a heist with other members!")
@app_commands.describe(target_amount="How many coins to try to steal")
async def slash_heist(interaction: discord.Interaction, target_amount: int = 500):
    eco = get_economy(interaction.user.id, str(interaction.user))
    if eco["balance"] < 100:
        return await interaction.response.send_message(
            embed=error_embed("Too Broke", "You need at least **100 coins** to start a heist!"), ephemeral=True
        )
    success_chance = random.random()
    embed = discord.Embed(
        title="🦝 Heist Starting!",
        description=(
            f"{interaction.user.mention} is planning a heist for **{target_amount:,} coins**!\n\n"
            f"React with 🤝 to join the crew!\n"
            f"**Launching in 30 seconds...**"
        ),
        color=0xff9f43
    )
    ...
    if len(participants) == 0:
        participants = [interaction.user]
    win_chance = min(0.3 + (len(participants) * 0.1), 0.8)
    if random.random() < win_chance:
        share = target_amount // len(participants)
        winners = []
        for p in participants:
            p_eco = get_economy(p.id, str(p))
            p_eco["balance"] += share
            p_eco["total_earned"] += share
            winners.append(p.mention)
        result_embed = discord.Embed(
            title="🎉 Heist Successful!",
            description=(
                f"The crew pulled it off! 💰\n\n"
                f"**Each member earned:** {share:,} coins\n"
                f"**Crew:** {', '.join(winners)}"
            ),
            color=0x00d4aa
        )
        ...
        for p in participants:
            p_eco = get_economy(p.id, str(p))
            p_eco["balance"] = max(0, p_eco["balance"] - fine)
            losers.append(p.mention)
        result_embed = discord.Embed(
            title="🚨 Heist Failed!",
            description=(
                f"The crew got caught! 🚔\n\n"
                f"**Each member lost:** {fine:,} coins\n"
                f"**Caught:** {', '.join(losers)}"
            ),
            color=0xff4d6d
        )
...
async def slash_shop(interaction: discord.Interaction):
    embed = discord.Embed(
        title="🛒 YBS Coin Shop",
        description="Spend your hard-earned coins here!\nUse `/buy <item>` to purchase.",
        color=0x6c63ff,
        timestamp=discord.utils.utcnow()
    )
    items = [
        ("🎭 Custom Role Colour", "customcolor", 5000, "Change your role colour for 7 days"),
        ("📢 Shoutout", "shoutout", 2000, "Get a shoutout in announcements"),
        ("🎁 Mystery Box", "mysterybox", 1000, "Random amount of coins (100-5000)"),
        ("⭐ XP Boost (24h)", "xpboost", 3000, "2x XP for 24 hours"),
        ("🔮 Fortune Teller", "fortune", 500, "3 custom fortune cookie messages"),
        ("🎰 Free Spins x5", "freespins", 750, "5 free slot machine spins"),
    ]
    for emoji_name, key, price, desc in items:
        embed.add_field(
            name=f"{emoji_name} — {price:,} coins",
            value=f"`/buy {key}` · {desc}",
            inline=False
        )
    embed.set_footer(text="YBS Economy · Earn coins with /daily and /work")
    await interaction.response.send_message(embed=embed)
 
@tree.command(name="buy", description="Buy an item from the shop")
@app_commands.describe(item="Item to buy")
@app_commands.choices(item=[
    app_commands.Choice(name="🎁 Mystery Box (1,000 coins)", value="mysterybox"),
    app_commands.Choice(name="⭐ XP Boost 24h (3,000 coins)", value="xpboost"),
    app_commands.Choice(name="🔮 Fortune Teller (500 coins)", value="fortune"),
    app_commands.Choice(name="🎰 Free Spins x5 (750 coins)", value="freespins"),
    app_commands.Choice(name="📢 Shoutout (2,000 coins)", value="shoutout"),
])
async def slash_buy(interaction: discord.Interaction, item: str):
    eco = get_economy(interaction.user.id, str(interaction.user))
    prices = {"mysterybox": 1000, "xpboost": 3000, "fortune": 500, "freespins": 750, "shoutout": 2000, "customcolor": 5000}
    price = prices.get(item, 0)
    if eco["balance"] < price:
        return await interaction.response.send_message(
            embed=error_embed("Insufficient Funds", f"You need **{price:,} coins** but only have **{eco['balance']:,}**."),
            ephemeral=True
        )
    eco["balance"] -= price
    if item == "mysterybox":
        ...
        eco["balance"] += reward
        eco["total_earned"] += reward
        embed = discord.Embed(
            title="🎁 Mystery Box Opened!",
            description=f"You spent **1,000 coins** and got back **{reward:,} coins**!\n{'🎉 Profit!' if reward > 1000 else '😅 Better luck next time!'}",
            color=0x6c63ff
        )
    elif item == "xpboost":
        if interaction.user.id not in premium_users:
            premium_users[interaction.user.id] = {}
        premium_users[interaction.user.id]["xp_boost_until"] = (datetime.now() + timedelta(hours=24)).isoformat()
        embed = discord.Embed(title="⭐ XP Boost Activated!", description="You have **2x XP** for the next **24 hours**! 🚀", color=0xffd166)
    elif item == "fortune":
        fortunes = [
            "Your next script will run perfectly first try.",
            "A breakthrough in your project is closer than you think.",
            "Your portfolio will impress someone important very soon.",
        ]
        embed = discord.Embed(title="🔮 Your Fortunes", description="\n\n".join(f"*\"{f}\"*" for f in fortunes), color=0x9b59b6)
    elif item == "freespins":
        embed = discord.Embed(title="🎰 Free Spins!", description="You got **5 free slot machine spins**!\nUse `/slots 0` and the shop will cover it! *(Feature coming soon)*", color=0xff9f43)
    elif item == "shoutout":
        ann_ch_id = get(interaction.guild.id, "announcements_channel")
        channel = bot.get_channel(ann_ch_id) if ann_ch_id else interaction.channel
        shout_embed = discord.Embed(
            title="📢 Member Shoutout!",
            description=f"Shoutout to {interaction.user.mention} for supporting Young Boy Studios! 🎉\n\nThey purchased a shoutout with their hard-earned coins! 💰",
            color=0x00d4aa
        )
        shout_embed.set_thumbnail(url=interaction.user.display_avatar.url)
        await channel.send(embed=shout_embed)
        embed = discord.Embed(title="📢 Shoutout Posted!", description=f"Your shoutout was posted in {channel.mention}!", color=0x00d4aa)
    else:
        embed = success_embed("Purchase Complete!", f"You bought **{item}** for **{price:,} coins**.")
...
async def slash_8ball(interaction: discord.Interaction, question: str):
    ...
    embed.set_footer(text="Young Boy Studios")
    await interaction.response.send_message(embed=embed)
...
async def slash_joke(interaction: discord.Interaction):
    jokes = [
        ...
        ("What's a pirate's fav language?", "R, matey! 🏴‍☠️"),
        ("Why was the JavaScript dev sad?", "Because he didn't Node how to Express himself!"),
    ]
    ...
    embed.set_footer(text="Young Boy Studios · Fun Commands")
    await interaction.response.send_message(embed=embed)
...
async def slash_rate(interaction: discord.Interaction, thing: str):
    ...
    color = 0x00d4aa if score >= 70 else 0xf39c12 if score >= 40 else 0xe74c3c
...
async def slash_compliment(interaction: discord.Interaction, member: discord.Member = None):
    ...
    cs = [
        ...
        "has the best scripts in the game! 💻", "could model anything! 🎨",
...
async def slash_truth(interaction: discord.Interaction):
    ts = [
        ...
        "Have you ever taken credit for someone else's script?",
    ]
    embed = discord.Embed(title="😳 Truth!", description=random.choice(ts), color=0xFF79C6)
    embed.set_footer(text="Young Boy Studios · Truth or Dare")
    await interaction.response.send_message(embed=embed)
 
@tree.command(name="dare", description="Get a random dare")
async def slash_dare(interaction: discord.Interaction):
    ds = [
        "Make a mini-game in 30 minutes and share it!",
        ...
        "DM someone a random Roblox development meme!",
        "Post a screenshot of your most embarrassing code!",
    ]
    embed = discord.Embed(title="😈 Dare!", description=random.choice(ds), color=0xe74c3c)
    embed.set_footer(text="Young Boy Studios · Truth or Dare")
    await interaction.response.send_message(embed=embed)
...
async def slash_coinflip(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title=f"🪙 Coin Flip — **{result}!**", color=0xf1c40f)
    await interaction.response.send_message(embed=embed)
...
async def slash_dice(interaction: discord.Interaction, sides: int = 6):
    ...
    embed = discord.Embed(title=f"🎲 D{sides} Roll — **{result}**", color=0x5865f2)
    await interaction.response.send_message(embed=embed)
...
async def slash_choose(interaction: discord.Interaction, options: str):
    ...
    embed = discord.Embed(title="🎯 I Choose...", description=f"**{pick}**", color=0x5865f2)
...
async def slash_pp(interaction: discord.Interaction, member: discord.Member = None):
    ...
    bar = "8" + "=" * score + "D"
    await interaction.response.send_message(f"📏 **{member.display_name}'s size:**\n`{bar}` ({score} cm)")
...
async def slash_iq(interaction: discord.Interaction, member: discord.Member = None):
    ...
    label = ("Genius 🧠" if score >= 160 else "Very Smart 🎓" if score >= 130
             else "Smart 📚" if score >= 110 else "Average 🙂" if score >= 90
             else "Hmm... 🤔" if score >= 70 else "Uh oh... 😬")
    embed = discord.Embed(title="🧠 IQ Test", description=f"**{member.display_name}**'s IQ is **{score}** — {label}", color=0x9b59b6)
    await interaction.response.send_message(embed=embed)
 
@tree.command(name="wouldyou", description="Would you rather question")
@app_commands.describe(option_a="Option A (leave blank for random)", option_b="Option B (leave blank for random)")
async def slash_wouldyou(interaction: discord.Interaction, option_a: str = None, option_b: str = None):
    presets = [
        ...
        ("Work solo forever", "Work in a team forever"),
        ("Be the best builder", "Be the best scripter"),
    ]
...
async def slash_nhie(interaction: discord.Interaction):
    questions = [
        ...
        "Never have I ever copied code from DevForum without understanding it.",
        ...
        "Never have I ever stayed up past 2am just to finish a feature.",
        "Never have I ever used free models in a serious project.",
    ]
    embed = discord.Embed(title="🤚 Never Have I Ever", description=random.choice(questions), color=0xFF79C6)
    embed.set_footer(text="🖐️ = Have done it | ☝️ = Never done it")
    msg = await interaction.response.send_message(embed=embed)
    msg = await interaction.original_response()
    await msg.add_reaction("🖐️")
    await msg.add_reaction("☝️")
...
async def slash_fortune(interaction: discord.Interaction):
    fortunes = [
        ...
        "Your portfolio will impress someone important soon.",
        "A breakthrough in your project is closer than you think.",
        "The best games are made by those who never stop learning.",
        "Your creativity is your greatest asset — use it boldly.",
    ]
    embed = discord.Embed(title="🥠 Fortune Cookie", description=f"*\"{random.choice(fortunes)}\"*", color=0xf1c40f)
    embed.set_footer(text="Young Boy Studios · Fortune Cookie")
    await interaction.response.send_message(embed=embed)
...
async def slash_riddle(interaction: discord.Interaction):
    ...
    await msg.add_reaction("💡")
    try:
        await bot.wait_for("reaction_add", timeout=30,
            check=lambda r, u: str(r.emoji) == "💡" and not u.bot and r.message.id == msg.id)
...
async def slash_trivia(interaction: discord.Interaction):
    questions = [
        ...
        ("What does RemoteEvent do?", "Communicates between server and client"),
    ]
...
@app_commands.describe(expression="Math expression e.g. 50 * 3 + 10")
async def slash_calc(interaction: discord.Interaction, expression: str):
    if not all(c in "0123456789+-*/.() " for c in expression):
        return await interaction.response.send_message(embed=error_embed("Invalid", "Only basic math operators allowed!"), ephemeral=True)
    try:
...
async def slash_afk(interaction: discord.Interaction, reason: str = "AFK"):
    ...
    embed = discord.Embed(description=f"💤 {interaction.user.mention} is now AFK: **{reason}**", color=0x6b7280)
    await interaction.response.send_message(embed=embed)
...
async def slash_remindme(interaction: discord.Interaction, duration: str, reminder: str):
    ...
    for value, unit in matches:
        multipliers = {"s": 1, "m": 60, "h": 3600, "d": 86400}
        total_seconds += int(value) * multipliers.get(unit, 0)
    remind_time = (datetime.now() + timedelta(seconds=total_seconds)).isoformat()
    ...
    ts = int((datetime.now() + timedelta(seconds=total_seconds)).timestamp())
    embed = discord.Embed(
        title="⏰ Reminder Set!",
...
@app_commands.describe(hex_code="Hex code e.g. 6c63ff")
async def slash_color(interaction: discord.Interaction, hex_code: str):
    ...
    try:
        ...
        embed.add_field(name="Decimal", value=f"`{int(hex_code, 16)}`")
        await interaction.response.send_message(embed=embed)
    except:
...
async def slash_suggest(interaction: discord.Interaction, suggestion: str):
    ...
    embed = discord.Embed(title="💡 New Suggestion", description=suggestion, color=0x00d4aa, timestamp=discord.utils.utcnow())
    embed.set_author(name=interaction.user.display_name, icon_url=interaction.user.display_avatar.url)
    msg = await channel.send(embed=embed)
    await msg.add_reaction("✅")
    await msg.add_reaction("❌")
    await interaction.response.send_message(embed=success_embed("Suggestion Submitted!", "Your suggestion has been posted!"), ephemeral=True)
...
@tree.command(name="addword", description="Add a word to auto-mod [Staff]")
...
async def slash_addword(interaction: discord.Interaction, word: str):
    ...
    await interaction.response.send_message(embed=success_embed("Word Added", f"`{word}` added to filter. Total: {len(automod_data[gid])}"), ephemeral=True)
 
@tree.command(name="removeword", description="Remove a word from auto-mod [Staff]")
...
async def slash_removeword(interaction: discord.Interaction, word: str):
    ...
    if match:
        wl.remove(match)
        await interaction.response.send_message(embed=success_embed("Removed", f"`{word}` removed."), ephemeral=True)
    else:
        await interaction.response.send_message(embed=error_embed("Not Found"), ephemeral=True)
 
@tree.command(name="wordlist", description="View auto-mod word filter [Staff]")
async def slash_wordlist(interaction: discord.Interaction):
    if not interaction.user.guild_permissions.manage_messages:
        return await interaction.response.send_message(embed=error_embed("No Permission"), ephemeral=True)
    gid = str(interaction.guild.id)
    words = automod_data.get(gid, [])
    embed = discord.Embed(title="🤖 Auto-Mod Filter", color=0xe74c3c)
    embed.description = ", ".join(f"`{w}`" for w in words) if words else "No words filtered."
    embed.set_footer(text=f"{len(words)} word(s)")
    await interaction.response.send_message(embed=embed, ephemeral=True)
 
@tree.command(name="giveaway", description="Start a giveaway [Staff]")
@app_commands.describe(prize="Prize description", duration_minutes="Duration in minutes")
async def slash_giveaway(interaction: discord.Interaction, prize: str, duration_minutes: int = 60):
    ...
    giveaway_data[msg.id] = {
        "prize": prize, "channel": channel.id,
        "ends": end_time.isoformat(), "host": str(interaction.user)
    }
    add_activity("🎉", f"Giveaway: {prize}", f"{duration_minutes}min by {interaction.user.display_name}")
    ...
    if msg.id in giveaway_data:
        try:
            ...
            if users:
                ...
                result_embed = discord.Embed(title="🎉 GIVEAWAY ENDED", description=f"Winner: {winner.mention}\nPrize: **{prize}**", color=0x57f287)
...
async def slash_serverrules(interaction: discord.Interaction):
    if not interaction.user.guild_permissions.manage_guild:
        return await interaction.response.send_message(embed=error_embed("No Permission"), ephemeral=True)
    embed = discord.Embed(title="📜 Server Rules", color=0x6c63ff, timestamp=discord.utils.utcnow())
    rules = [
        ...
        "Listen to and respect all staff decisions.",
        "Do not share personal information without consent.",
        "Have fun, be creative, and help us build something great! 🎮",
    ]
    embed.description = "\n".join(f"**{i+1}.** {r}" for i, r in enumerate(rules))
    embed.set_footer(text="Violation of rules may result in mutes, kicks, or bans.")
    await interaction.channel.send(embed=embed)
...
async def slash_birthday(interaction: discord.Interaction, month: int, day: int):
    ...
    embed = discord.Embed(
        title="🎂 Birthday Set!",
        description=f"Your birthday is set to **{month}/{day}**!\nWe'll celebrate on your special day! 🎉",
        color=0xFF79C6
    )
    await interaction.response.send_message(embed=embed, ephemeral=True)
 
@tree.command(name="counting-stats", description="View counting stats")
async def slash_counting_stats(interaction: discord.Interaction):
    guild_id = str(interaction.guild.id)
    data = counting_data.get(guild_id, {"count": 0, "highscore": 0})
...
async def slash_counting_reset(interaction: discord.Interaction):
    ...
    if guild_id in counting_data:
        counting_data[guild_id]["count"] = 0
        counting_data[guild_id]["last_user"] = None
...
async def slash_setup(interaction: discord.Interaction):
    if not interaction.user.guild_permissions.administrator:
        return await interaction.response.send_message(embed=error_embed("Admin Only"), ephemeral=True)
    embed = discord.Embed(title="⚙️ Bot Setup", description="Configure channels, roles, and features below.", color=0x6c63ff)
...
async def slash_premium(interaction: discord.Interaction, member: discord.Member, days: int = 30):
    if not interaction.user.guild_permissions.administrator:
        return await interaction.response.send_message(embed=error_embed("Admin Only"), ephemeral=True)
    premium_users[member.id] = {
        "expires": (datetime.now() + timedelta(days=days)).isoformat(),
        "tier": "premium", "granted_by": str(interaction.user)
    }
    prem_role_id = get(interaction.guild.id, "premium_role")
    if prem_role_id:
        prem_role = interaction.guild.get_role(prem_role_id)
        if prem_role:
            try:
                await member.add_roles(prem_role)
            except:
                pass
    try:
        await member.send(embed=discord.Embed(
            title="👑 Premium Granted!",
            description=f"You've been granted **Premium** in **{interaction.guild.name}** for **{days} days**!\n\n✅ 1.5x XP boost\n✅ Bonus daily coins\n✅ Premium channels access",
            color=0xffd166
        ))
    except:
        pass
...
async def slash_reactionrole(interaction: discord.Interaction, message_id: str, emoji: str, role: discord.Role):
    ...
    if gid not in reaction_roles:
        reaction_roles[gid] = {}
    if message_id not in reaction_roles[gid]:
        reaction_roles[gid][message_id] = {}
...
async def slash_unlockdown(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description=f"**{count}** channels unlocked by {interaction.user.mention}.",
        color=0x00d4aa,
        timestamp=discord.utils.utcnow()
    )
    await interaction.followup.send(embed=embed)
...
async def slash_shutdown(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description=f"To confirm shutdown use `/shutdown-confirm` with code:\n```\n{code}\n```\n**This will take the bot offline!**",
        color=0xe74c3c
    )
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def setup(ctx):
    embed = discord.Embed(title="⚙️ Young Boy Studios — Bot Setup", description="Use the buttons below to configure the bot.", color=0x6c63ff)
...
async def showconfig(ctx):
    ...
    if not config:
        return await ctx.send(embed=error_embed("No Config", "Run `!setup` first!"))
    embed = discord.Embed(title="⚙️ Current Config", color=0x6c63ff)
    for key, value in config.items():
...
@bot.command()
async def help(ctx):
    embed = discord.Embed(title="📚 YBS Bot — Commands", color=0x6c63ff)
    embed.add_field(name="⚙️ Setup", value="`!setup` `!showconfig`", inline=False)
    embed.add_field(name="🔨 Moderation", value="`!kick` `!ban` `!warn` `!purge` `!mute` `!unmute` `!lock` `!unlock`", inline=False)
    embed.add_field(name="ℹ️ Info", value="`!userinfo` `!serverinfo` `!ping` `!membercount`", inline=False)
    embed.add_field(name="💡 Slash Commands", value="Use `/help` for all 80+ slash commands!", inline=False)
    embed.set_footer(text="Young Boy Studios Bot · Prefix: !")
    await ctx.send(embed=embed)
 
@bot.command()
async def ping(ctx):
    await ctx.send(embed=discord.Embed(title="🏓 Pong!", description=f"**{round(bot.latency * 1000)}ms**", color=0x00d4aa))
 
@bot.command()
async def userinfo(ctx, member: discord.Member = None):
    member = member or ctx.author
    embed = discord.Embed(title=f"👤 {member}", color=member.color if str(member.color) != "#000000" else 0x6c63ff)
    embed.set_thumbnail(url=member.display_avatar.url)
    embed.add_field(name="ID", value=member.id)
    embed.add_field(name="Joined", value=member.joined_at.strftime("%d/%m/%Y") if member.joined_at else "?")
...
async def serverinfo(ctx):
    g = ctx.guild
    embed = discord.Embed(title=f"🏠 {g.name}", color=0x6c63ff)
...
async def membercount(ctx):
    await ctx.send(embed=discord.Embed(description=f"👥 **{ctx.guild.member_count}** members!", color=0x6c63ff))
 
@bot.command()
async def dice(ctx):
    await ctx.send(embed=discord.Embed(title=f"🎲 You rolled a **{random.randint(1,6)}**!", color=0x5865f2))
 
@bot.command()
async def coinflip(ctx):
    await ctx.send(embed=discord.Embed(title=f"🪙 **{random.choice(['Heads','Tails'])}!**", color=0xf1c40f))
 
@bot.command()
@commands.has_permissions(kick_members=True)
async def kick(ctx, member: discord.Member, *, reason="No reason"):
    await member.kick(reason=reason)
    await ctx.send(embed=success_embed("Kicked", f"**{member}** kicked. Reason: {reason}"))
...
async def ban(ctx, member: discord.Member, *, reason="No reason"):
    ...
    await ctx.send(embed=success_embed("Banned", f"**{member}** banned. Reason: {reason}"))
...
async def purge(ctx, amount: int):
    ...
    await ctx.send(embed=success_embed("Purged", f"Deleted **{amount}** messages."), delete_after=3)
...
async def warn(ctx, member: discord.Member, *, reason="No reason"):
    if not is_staff(ctx):
        return await ctx.send(embed=error_embed("No Permission"))
    ...
    count = len(warnings_data[member.id])
    await ctx.send(embed=success_embed("Warned", f"**{member}** warned ({count}/3). Reason: {reason}"))
...
async def mute(ctx, member: discord.Member, minutes: int = 10, *, reason="No reason"):
    ...
    try:
        await member.timeout(discord.utils.utcnow() + timedelta(minutes=minutes), reason=reason)
        await ctx.send(embed=success_embed("Muted", f"**{member}** muted for **{minutes}min**."))
        add_mod_log("Mute", str(member), str(ctx.author), f"{minutes}min — {reason}", "#faa61a")
    except:
...
async def unmute(ctx, member: discord.Member):
    ...
    try:
        await member.timeout(None)
        await ctx.send(embed=success_embed("Unmuted", f"**{member}** unmuted."))
    except:
        await ctx.send(embed=error_embed("Failed"))
...
def common():
    ...
    for uid, warns in list(warnings_data.items())[-8:]:
        if warns:
            recent_warns.append({
                "user_id": uid, "count": len(warns),
                "last_reason": warns[-1]["reason"], "last_by": warns[-1].get("by", "—")
            })
    return dict(
        ...
        mod_action_counts=get_mod_action_counts(),
    )
...
def dashboard_notes():
    return render_template("dashboard.html", page="notes", all_notes={str(k): v for k, v in notes_data.items()}, **common())
...
def dashboard_leaderboard():
    return render_template("dashboard.html", page="leaderboard",
        xp_lb=build_xp_leaderboard()[:25], eco_lb=build_eco_leaderboard()[:25], **common())
...
def dashboard_automod():
    return render_template("dashboard.html", page="automod",
        automod_guilds=automod_data, total_words=sum(len(v) for v in automod_data.values()), **common())
...
def dashboard_economy():
    ...
    return render_template("dashboard.html", page="economy",
        eco_lb=eco_lb[:25], total_coins=total_coins,
        total_earned_ever=total_earned_ever, richest=richest, **common())
...
def dashboard_roblox():
    accounts = [{
        ...
        "username": v["username"], "display": v.get("display", v["username"]),
        "roblox_id": v["roblox_id"], "thumb": v.get("thumb"),
        "linked_at": v.get("linked_at", "—"), "verified": v.get("verified", False)
...
def dashboard_premium():
    return render_template("dashboard.html", page="premium",
        premium_members={str(k): v for k, v in premium_users.items()}, **common())
